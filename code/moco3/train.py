...
import models
from losses import ctr
from dataloader import VideoDataset, Augmentator
...
def momentum_update(model_q, model_k, m):
    with torch.no_grad():
        for param_q, param_k in zip(model_q.module.parameters(), model_k.module.parameters()):
...
def train(
    ...
    model_q = nn.parallel.DistributedDataParallel(
        model_q,
        device_ids=[device.index],
        output_device=device.index,
        broadcast_buffers=False
    )
    model_k = nn.parallel.DistributedDataParallel(
        model_k,
        device_ids=[device.index],
        output_device=device.index,
        broadcast_buffers=False
    ...
    aug_worker = Augmentator(device=device)

    m = 0.99
    optimizer = torch.optim.AdamW(model_q.parameters(), lr=0.0001)
    epoch_losses = []

    debug_shape_printed = False
    train_start = time.time()

    # 디버깅용: 문제 해결 후 False로 바꾸거나 제거 가능
    ...
    for epoch in range(epochs):
        ...
        for _, data in enumerate(progress_bar):
            ...
            if rank == 0 and not debug_shape_printed:
                print(f"[Debug] First raw batch shape: {tuple(data.shape)}")
            ...
            flat_data = data.reshape(current_batch_size * param_frame, 3, param_img_size, param_img_size)

            x1 = aug_worker.aug(flat_data).reshape(
                current_batch_size, param_frame, 3, param_img_size, param_img_size
            )
            x2 = aug_worker.aug(flat_data).reshape(
                current_batch_size, param_frame, 3, param_img_size, param_img_size
            )

            optimizer.zero_grad(set_to_none=True)

            # query encoder: 두 view를 한 번에 forward
            q = model_q(torch.cat([x1, x2], dim=0))
            q1, q2 = torch.chunk(q, chunks=2, dim=0)

            # key encoder 업데이트 후 두 view를 한 번에 forward
            momentum_update(model_q, model_k, m)
            with torch.no_grad():
                k = model_k(torch.cat([x1, x2], dim=0))
                k1, k2 = torch.chunk(k, chunks=2, dim=0)
    ...
    if rank == 0:
        ...
        torch.save(model_q.module.state_dict(), path_dict["ckpt"])
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ViViT",
        choices=["ViViT", "R3D18", "MC3_18", "R2Plus1D", "C3D", "X3D"]
    )
    parser.add_argument("--framework_name", type=str, default="moco3")
    parser.add_argument("--save_dir", type=str, default="./experiments/moco3_ViViT")
